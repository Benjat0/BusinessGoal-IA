  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-[30px] border border-slate-800 bg-[#050B16] shadow-2xl">
        ...
        <div className="flex flex-col gap-3 border-t border-slate-800 p-5 sm:flex-row sm:justify-between"><button disabled={props.wizardStep === 1} onClick={() => props.setWizardStep(Math.max(1, props.wizardStep - 1) as WizardStep)} className="rounded-xl border border-slate-800 px-4 py-3 text-sm font-bold text-slate-300 disabled:cursor-not-allowed disabled:opacity-40">Anterior</button><div className="flex flex-wrap gap-3">{props.wizardStep === 2 && <button disabled={!props.uploadReady || props.isInspecting} onClick={props.onInspect} className="rounded-xl bg-slate-800 px-4 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40">{props.isInspecting ? "Inspeccionando..." : "Inspeccionar archivos"}</button>}{props.wizardStep < 5 && ![2,3,4].includes(props.wizardStep) && <button onClick={() => props.setWizardStep(Math.min(5, props.wizardStep + 1) as WizardStep)} className="rounded-xl border border-slate-800 px-4 py-3 text-sm font-bold text-slate-300">Continuar</button>}{props.wizardStep === 3 && <button onClick={() => props.setWizardStep(4)} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-black text-white">Validar columnas</button>}{props.wizardStep === 4 && <button onClick={() => props.setWizardStep(5)} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-black text-white">Continuar a confirmar</button>}{props.wizardStep === 5 && <button disabled={!props.inspection?.analysis_ready || props.isAnalyzing} onClick={props.onAnalyze} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40">{props.isAnalyzing ? "Generando..." : "Generar análisis"}</button>}</div></div>
      </div>
    </div>
  );
}
